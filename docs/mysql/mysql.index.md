...
```
[root@leco ~]# systemctl start mariadb #启动
[root@leco ~]# systemctl enable mariadb #设置开机自启动
Created symlink from /etc/systemd/system/multi-user.target.wants/mariadb.service to /usr/lib/systemd/system/mariadb.service.
[root@leco ~]# ps aux |grep mysqld |grep -v grep #查看进程，mysqld_safe为启动mysql的脚本文件，内部调用mysqld命令
...
[root@leco ~]# netstat -an |grep 3306 #查看端口
tcp        0      0 0.0.0.0:3306            0.0.0.0:*               LISTEN  
[root@leco ~]# ll -d /var/lib/mysql #权限不对，启动不成功，注意user和group
drwxr-xr-x 5 mysql mysql 4096 Jul 20 16:28 /var/lib/mysql
```
You must reset your password using ALTER USER statement before executing this statement.

```
装完mysql 之后，登陆以后，不管运行任何命令，总是提示这个
...
```
[root@leco ~]# rm -rf /var/lib/mysql/mysql #所有授权信息全部丢失！！！
[root@leco ~]# systemctl restart mariadb
[root@leco ~]# mysql
```
方法二：启动时，跳过授权库
...
设置密码
[root@leco ~]# mysqladmin -uroot password "123"        设置初始密码 由于原密码为空，因此-p可以不用
[root@leco ~]# mysqladmin -uroot -p"123" password "456"        修改mysql密码,因为已经有密码了，所以必须输入原密码才能设置新密码

命令格式:
[root@leco ~]# mysql -h172.31.0.2 -uroot -p456
[root@leco ~]# mysql -uroot -p
[root@leco ~]# mysql                    以root用户登录本机，密码为空
```
