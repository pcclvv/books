<center><h1> MySQL 配置文件 </h1></center>

## 1. 介绍
&#160; &#160; &#160; &#160;对于MySQL的理解，我认为很多性能优化工作、主从主主复制都是在调整参数，来适应不同时期不同数量级的数据。故，理解透彻my.cnf里的参数是永恒的话题；只有理解透彻了参数设置，才能在某些方面对数据库进行调优。
...
log-bin = /var/log/mysql/mysql-bin.log	　　　　　　　        # 开启二进制日志功能，binlog数据位置
log-bin-index = /var/log/mysql/mysql-bin.index
relay-log = /var/log/mysql/relay/mysql-relay-bin	　　　    # relay-log日志记录的是从服务器I/O线程将主服务器的二进制日志读取过来记录到从服务器本地文件，
　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　# 然后SQL线程会读取relay-log日志的内容并应用到从服务器
relay-log-index = /var/log/mysql/relay/mysql-relay-bin.index  # binlog传到备机被写道relaylog里，备机的slave sql线程从relaylog里读取然后应用到本地。
...
auto_increment_offset = 1                   # 漂移值,也就是步长
auto_increment_increment = 2　　　　　　　　# 自增值,这两个参数一般用在主主同步中，用来错开自增值, 防止键值冲突
...
innodb_log_buffer_size = 256M　　　　　　   # nnoDB 将日志写入日志磁盘文件前的缓冲大小。理想值为 1M 至 8M。大的日志缓冲允许事务运行时不需要将日志保存入磁盘而只到事务被提交(commit)。
